...
class Layer:
    ...
    def __init__(self, num_inputs, num_outputs, activation=linear, bias=True, trainable=True):
        ...
        self.weights = np.random.normal(0, 1 / np.sqrt(self.num_inputs), size=(num_inputs, num_outputs)).astype(np.float32)
    ...
    def __call__(self, inputs, d=False):
        return self.activation(inputs @ self.weights + self.biases, d=d)
